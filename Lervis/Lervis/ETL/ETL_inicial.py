...
logger = crear_logger('ETL_inicial', 'ETL_inicial.log')
...
for publicacion in metadatos_publicaciones.itertuples():
    try:
        ...
        fin = time.time()
        duracion = fin - inicio
        logger.debug(f'ETL inicial finalizado con exito - Duracion segundos: {duracion:.2f} - Total Publicaciones: {total_publicaciones} - Total Chunks: {total_fragmentos} - Total Embeddings: {total_embeddings}')
    except Exception as e:
        logger.error(f'Error en el pipeline de extraccion: {e} -Duracion segundos: {duracion:.2f} - Total Publicaciones: {total_publicaciones} - Total Chunks: {total_fragmentos} - Total Embeddings: {total_embeddings}')
